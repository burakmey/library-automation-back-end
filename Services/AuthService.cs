...
namespace library_automation_back_end.Services
{
    public class AuthService(IConfiguration configuration, DataContext dataContext)
    {
        ...
        public async Task<MessageResponse> CreateUser(RegisterRequest request)
        {
            ...
            return new() { Succeeded = true, Message = "Registration is completed." };
        }
        ...
        public async Task<User?> GetUser(LoginRequest request)
        {
            User? user = await dataContext.Users.Include(u => u.Country).Include(u => u.Role).Include(u => u.Fines)
                .Include(u => u.UserBookBorrows).ThenInclude(ubb => ubb.Book)
                .Include(u => u.UserBookReserves).ThenInclude(ubr => ubr.Book)
        ...
        {
            User? user = await dataContext.Users.FirstOrDefaultAsync(u => u.Email == email);
            return user!.Id;
        }
